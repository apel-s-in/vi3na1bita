...
// Безопасные настройки Sharp: убираем кэш и параллелизм, чтобы исключить
// «залипание» предыдущего входного изображения между конвертациями.
sharp.cache(false);
sharp.concurrency(1);
...
async function regenerateDerivative(srcPath, targetPath, kind) {
  // Перегенерируем производные гарантированно из своего источника.
  // Удаляем цель заранее и создаём НОВЫЙ пайплайн sharp для каждого файла.
  try {
